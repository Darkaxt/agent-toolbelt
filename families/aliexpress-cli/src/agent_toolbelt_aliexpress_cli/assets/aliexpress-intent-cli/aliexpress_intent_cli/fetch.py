...
def is_probably_blocked_html(html: str, *, status: int | None = None) -> bool:
    ...
    return bool(
        ...
        or "sign in to view" in lowered
        or "punish" in lowered
        or "secureshopping" in lowered and "login" in lowered
...
class AliExpressFetcher:
    ...
    def fetch(self, url: str, *, timeout_sec: int = 30) -> FetchResult:
        ...
        if not self.curl_executable:
            raise FetchError("AliExpress fetch failed and curl is not available.")
        ...
        if is_probably_blocked_html(result.html, status=result.status):
            result.warnings.append("AliExpress response appears blocked, challenged, or login-gated.")
        return result
    ...
    def _fetch_curl(self, url: str, *, timeout_sec: int) -> FetchResult:
        ...
        if marker in completed.stdout:
            html, status_text = completed.stdout.rsplit(marker, 1)
            try:
