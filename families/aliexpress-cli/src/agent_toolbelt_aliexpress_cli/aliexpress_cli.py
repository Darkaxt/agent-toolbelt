...
CLIENT_HOME_ENV = "ALIEXPRESS_INTENT_CLI_HOME"
CLIENT_FOLDER_NAME = "aliexpress-intent-cli"
CLIENT_ENTRYPOINT = "aliexpress-cli"
...
def bundled_client_home() -> Path:
    return package_root() / "assets" / "aliexpress-intent-cli"


def runtime_venv_dir() -> Path:
    local_appdata = os.getenv("LOCALAPPDATA")
    if local_appdata:
        return Path(local_appdata) / "agent-toolbelt" / "aliexpress-cli" / "uv-env"
    cache_home = os.getenv("XDG_CACHE_HOME")
    root = Path(cache_home).expanduser() if cache_home else Path.home() / ".cache"
    return root / "agent-toolbelt" / "aliexpress-cli" / "uv-env"
...
def parse_payload(stdout: str | None) -> dict[str, Any] | None:
    if stdout is None:
        return None
...
def collect_payload_warnings(payload: dict[str, Any] | None) -> list[str]:
...
def normalize_payload(
    *,
    ...
    result = payload if payload is not None else ({"raw_stdout": raw_stdout.strip()} if raw_stdout.strip() else {})
    return make_result(
        ok=exit_code == 0,
        operation=operation,
        result=result,
    ...
    *,
    ...
    if resolved_home is None:
        return make_result(
            ...
            operation=operation,
            stderr=(
                "AliExpress CLI client not available. "
                "Restore the bundled client project root, set the "
                f"{CLIENT_HOME_ENV} environment override, or pass --client-home. "
                "The legacy %LOCALAPPDATA%\\Tools project root remains a compatibility fallback."
            ),
            exit_code=127,
        )
    uv_executable = resolve_uv_executable()
    if uv_executable is None:
        return make_result(ok=False, operation=operation, stderr="uv is not available on PATH.", exit_code=127)
    ...
    try:
        ...
        return make_result(
            ok=False,
            operation=operation,
            stderr=merge_messages(exc.stderr or "", "AliExpress CLI client timed out."),
            exit_code=124,
        )
    try:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Bridge into the local AliExpress CLI client.")
    parser.add_argument("--client-home", help="Path to the standalone aliexpress-intent-cli project.")
    parser.add_argument("--timeout-sec", type=int, default=DEFAULT_TIMEOUT_SEC)
    parser.add_argument(
        "operation_args",
        nargs=argparse.REMAINDER,
        help="Arguments passed through to aliexpress-cli. Use `--` before aliexpress-cli flags.",
    )
    return parser
