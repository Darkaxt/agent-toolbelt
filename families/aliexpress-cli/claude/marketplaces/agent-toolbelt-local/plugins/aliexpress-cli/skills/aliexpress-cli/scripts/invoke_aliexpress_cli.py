...
def bootstrap_family_package() -> None:
    ...
    for candidate in candidate_repo_roots(current):
        ...
        if pyproject.exists() and "[tool.uv.workspace]" in pyproject.read_text(encoding="utf-8"):
            if not (family_src / "agent_toolbelt_aliexpress_cli").is_dir():
                continue
            ...
            return
    raise RuntimeError("Could not locate the agent-toolbelt aliexpress-cli package.")


bootstrap_family_package()

from agent_toolbelt_aliexpress_cli import cli  # noqa: E402
