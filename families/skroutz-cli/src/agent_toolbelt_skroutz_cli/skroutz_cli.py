...
CLIENT_HOME_ENV = "SKROUTZ_INTENT_CLI_HOME"
CLIENT_FOLDER_NAME = "skroutz-intent-cli"
CLIENT_ENTRYPOINT = "skroutz-cli"
...
def bundled_client_home() -> Path:
    return package_root() / "assets" / "skroutz-intent-cli"


def runtime_venv_dir() -> Path:
    local_appdata = os.getenv("LOCALAPPDATA")
    if local_appdata:
        return Path(local_appdata) / "agent-toolbelt" / "skroutz-cli" / "uv-env"
    cache_home = os.getenv("XDG_CACHE_HOME")
    root = Path(cache_home).expanduser() if cache_home else Path.home() / ".cache"
    return root / "agent-toolbelt" / "skroutz-cli" / "uv-env"
    ...
    *,
    ...
    if resolved_home is None:
        return make_result(
            ...
            operation=operation,
            stderr=(
                "Skroutz CLI client not available. "
                f"Restore the bundled client, set {CLIENT_HOME_ENV}, or pass --client-home."
            ),
            exit_code=127,
        )
    ...
    try:
        ...
        return make_result(
            ok=False,
            operation=operation,
            stderr=merge_messages(exc.stderr or "", "Skroutz CLI client timed out."),
            exit_code=124,
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Bridge into the local Skroutz CLI client.")
    parser.add_argument("--client-home", help="Path to the standalone skroutz-intent-cli project.")
    parser.add_argument("--timeout-sec", type=int, default=DEFAULT_TIMEOUT_SEC)
    parser.add_argument(
        "operation_args",
        nargs=argparse.REMAINDER,
        help="Arguments passed through to skroutz-cli. Use `--` before skroutz-cli flags.",
    )
    return parser
