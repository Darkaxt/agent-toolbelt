...
- Use `edit-draft --body/--subject/--to/--cc/--bcc/--attach ... --confirm` to update an existing Drafts-folder item. Read or locate the draft first, then call `edit-draft`. Do not update draft bodies through ad hoc COM; do not update recipients, subjects, or attachments through ad hoc COM either. The helper refuses to edit messages that are not in the selected account's Drafts folder.
