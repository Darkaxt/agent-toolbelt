...
- For "latest emails from X" or similar sender/service lookups: Search the cached folder inventory first with `find-folders`, then search matching folders.
...
- COM-backed commands acquire the FIFO queue and COM lock before applying Outlook startup policy. In an interactive desktop invocation, the helper starts Outlook Classic normally when it is absent and requests desktop promotion when only a hidden COM `-Embedding` instance exists. It never kills Outlook or an active helper process automatically.
- `diagnostics-probe` is launch-free. When Outlook is stopped it reports `outlook_not_running` rather than starting a hidden COM server. Background/`pythonw.exe` invocations also do not launch Outlook; they report `outlook_interactive_session_required` unless an Outlook process already exists. Set `OUTLOOK_CLASSIC_MAIL_BACKGROUND=1` or `OUTLOOK_CLASSIC_MAIL_NO_UI=1` for any explicitly background launcher.
- Inspect `client_diagnostics.outlook_startup` when startup is relevant. Its action distinguishes `existing_visible_instance`, `desktop_launch_requested`, `desktop_promotion_requested`, `probe_no_launch`, and `background_no_launch`.
- Do not use `taskkill`, `Stop-Process`, or ad hoc COM shutdown as a normal recovery step. Process termination requires an explicit user request after identifying the exact active wrapper/client PIDs and confirming that mailbox work may be interrupted.
- The FIFO queue reclaims non-expired tickets only when the recorded owner PID is provably dead. Inspect `queue.reclaimed_dead_tickets` after an explicitly interrupted operation; do not manually delete the queue database or inflate the lease wait.
...
- When attachment files from an existing message are needed locally, use `save-attachments --account <smtp|store> --message-id <entry-id> --output-dir <directory>`. Do not use ad hoc COM to export attachments.
...
- For threaded drafts, `--account` resolves the original message. When `--send-using-account` is omitted, the helper derives the sender from the anchor message's original recipient SMTP addresses if exactly one configured Outlook account matches, even when that account differs from the anchor store. Explicit `--send-using-account` overrides derivation. Multiple configured matches fail closed and require an explicit sender; no match falls back to the anchor account with `send_using_account_recipient_unmatched`.
- Use `draft-reply --reply-mode all` when the user wants all Outlook thread recipients. The helper normalizes Outlook native reply-all recipients to SMTP addresses when Outlook exposes them through the Recipients collection. Use explicit `--to`, `--cc`, or `--bcc` when the user names a specific recipient set; those explicit values must contain SMTP email addresses, not display names. Do not create a draft and then patch recipients through ad hoc COM.
...
- Pass actual multiline plain text and actual Unicode characters in `--body`. HTML-like tags and literal escape sequences such as `\n`, `\r`, `\t`, `\uXXXX`, or `\xNN` are not decoded. The helper still performs the requested action but emits an uppercase `WARNING:`; treat that as a formatting defect and do not claim the draft is correctly formatted until the body is corrected and re-read.
...
- After creating a reply/forward draft, inspect `send_using_account_selection.source`, `send_using_account_selection.selected`, `draft_content.thread_content_included`, `draft_content.thread_content_source`, `draft_placement.actual_send_using_account`, `draft_placement.placement_verified`, `draft_recipients.actual`, and `draft_attachments.items[].attached` before telling the user the draft is correctly threaded, using the intended sender, has the intended recipients, and has the requested files.
...
- For "move/file/put this email in folder X" tasks, use cache-backed `find-folders` first when the target is ambiguous. Use `--rediscover-folders` only if the cached inventory cannot locate the target. Run `move-message` without `--confirm` as a preview, and run `move-message --confirm` only after explicit user approval.
...
python scripts/invoke_outlook_mail.py [--queue-timeout-sec <n>] sync-mail [--refresh-cache] [--account <smtp|store>|--all-accounts] [--days <n>] [--force] [--rediscover-folders]
...
python scripts/invoke_outlook_mail.py [--queue-timeout-sec <n>] cache-refresh [--account <smtp|store>|--all-accounts] [--days <n>] [--force] [--rediscover-folders]
python scripts/invoke_outlook_mail.py cache-clear [--query <text>] --confirm
python scripts/invoke_outlook_mail.py [--queue-timeout-sec <n>] find-folders --query <text> [--account <smtp|store>|--all-accounts] [--limit <n>] [--rediscover-folders]
...
python scripts/invoke_outlook_mail.py [--queue-timeout-sec <n>] read-message --account <smtp|store> --message-id <entry-id> [--include-html]
python scripts/invoke_outlook_mail.py [--queue-timeout-sec <n>] save-attachments --account <smtp|store> --message-id <entry-id> --output-dir <directory>
...
python scripts/invoke_outlook_mail.py [--queue-timeout-sec <n>] draft-reply --account <smtp|store> [--send-using-account <smtp|store>] --message-id <entry-id> [--reply-mode sender|all] [--to <recipient>] [--cc <recipient>] [--bcc <recipient>] --instruction "<guidance>" [--body "<final draft text>" [--attach <local-file>] --create-draft --confirm]
python scripts/invoke_outlook_mail.py [--queue-timeout-sec <n>] draft-forward --account <smtp|store> [--send-using-account <smtp|store>] --message-id <entry-id> --to "<recipient>" [--cc <recipient>] [--bcc <recipient>] --instruction "<guidance>" [--body "<final draft text>" [--attach <local-file>] --create-draft --confirm]
python scripts/invoke_outlook_mail.py [--queue-timeout-sec <n>] edit-draft --account <smtp|store> --message-id <draft-entry-id> [--body "<final draft text>"] [--subject <subject>] [--to <recipient>] [--cc <recipient>] [--bcc <recipient>] [--attach <local-file>] --confirm
